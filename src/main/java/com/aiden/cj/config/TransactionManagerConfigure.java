...
/**
 * Spring声明式事物拦截器配置
 * 需要使用通用事务拦截，Application上面需要开启@EnableTransactionManagement
 * 不使用Springboot提供的注解事物方式@Transactional
 */
@Configuration
//Application上面需要开启@EnableTransactionManagement，通用事务配置才生效
@ConditionalOnBean(annotation = { EnableTransactionManagement.class })
